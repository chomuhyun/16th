...
AttackBoost::AttackBoost(std::string n, int increase, int Numberof) // 공격력 부스트(이름, 증가량, 수량)
	:name(n), attackIncrease(increase), numberofAttackBoost(Numberof){
}
...
HealthPotion::HealthPotion(std::string n, int restore, int Numberof) // 회복 포션(이름, 회복량, 수량)
	:name(n), healthRestore(restore), numberofHealthPotion(Numberof){
}
...
ExperienceBoost::ExperienceBoost(std::string n, int Increase, int Numberof) // 경험치 부스트(이름, 증가량, 수량)
	:name(n), experienceIncrease(Increase), numberofExperienceBoost(Numberof){
}
